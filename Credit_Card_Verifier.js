...
const validateCred = arr => {
  // let doubledArr = [].concat(arr);
  let doubledArr = arr.slice();
  for (let i = arr.length - 2; i >= 0; i -= 2) {
    doubledArr[i] *= 2;
  }
  doubledArr = doubledArr.map(num => num > 9 ? num - 9: num);
  let sum = 0;
  doubledArr.forEach(num => sum += num);
  if (sum % 10 === 0) {
    return true;
  }
  return false;
...
const findInvalidCards = cardArr => {
  let invalidArr = [];
  // console.log(cardArr);
  cardArr.forEach(card => !validateCred(card) ? invalidArr.push(card) : false);
  return invalidArr;
}
...
const idInvalidCardCompanies = invalidArr => {
  companyArr = [];
  for (const number in invalidArr) {
    switch (number[0]) {
      case 3:
        if (!companyArr.includes('Amax')) {
          companyArr.push('Amax');
        }
      case 4:
